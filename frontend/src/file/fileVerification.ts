	...
	public async getFileOwnerByHash(fileHash: string): Promise<string> {
		...
		const result = await this.aptos.view({
			payload: {
				...
				typeArguments: [],
				functionArguments: [this.registryAdmin, bytes],
			},
		});
	...
	public async getFileHistory(
		...
		const result = await this.aptos.view({
			payload: {
				...
				typeArguments: [],
				functionArguments: [owner, hexToByteArray(rootHash)],
			},
		});
		...
		return {
			versions: data.value.versions.map((v: number[]) =>
...
export const createFileVerificationService = (
