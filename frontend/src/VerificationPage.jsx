	...
	const handleVerify = async (e) => {
		...
		if (inputHash && !isValidHex(inputHash)) {
			setVerificationResult(
				"❌ Invalid hash format. Please enter a valid hexadecimal value."
			);
			return;
		}
		...
		try {
			...
			const record = await fileVerificationService.getFileRecordByHash(
				hashToVerify
			);
		...
		>
			...
			<div style={{ display: "grid", gap: "1rem" }}>
				...
				<DetailRow
					...
					monospace
				/>
				<DetailRow label="Root Hash" value={record.root_hash} monospace />
				<DetailRow
					label="Timestamp"
			...
			{verificationResult && (
				<div
					style={{
						...
						fontWeight: "500",
						padding: "1rem",
						backgroundColor: verificationResult.includes("✅")
							? "rgba(6, 78, 59, 0.2)"
							: "rgba(185, 28, 28, 0.2)",
						...
						display: "flex",
						alignItems: "center",
						border: verificationResult.includes("✅")
							? "1px solid rgba(16, 185, 129, 0.3)"
							: "1px solid rgba(239, 68, 68, 0.3)",
						boxShadow: verificationResult.includes("✅")
